...
public:
    ...
    void transformPatchCloud (pcl::PointCloud<pcl::PointXYZ>& input_patch_cloud, const Eigen::Matrix3f& matrix, pcl::PointCloud<pcl::PointXYZ>& transformed_cloud) const
    {
        ...
        for (unsigned int index_t = 0; index_t < number_of_points; index_t++)
        {
            Eigen::Vector3f transformed_point (
                        input_patch_cloud.points[index_t].x ,
                        input_patch_cloud.points[index_t].y ,
                        input_patch_cloud.points[index_t].z );
